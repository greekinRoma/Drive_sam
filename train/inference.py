...
from resize_tool_box.crop_grid import crop_image_grid, reconstruct_from_grid
from resize_tool_box.crop import crop_images, reconstruct_image
from resize_tool_box.resize import reconstruct_from_resize,resize_img
from resize_tool_box.crop_with_overlap import reconstruct_from_grid_with_overlap,crop_image_grid_with_overlap
...
def evaluate(args, net, sam, valid_dataloader, accelerator, visualize=True):
    ...
    for _,data in enumerate(tqdm(valid_dataloader,disable=not accelerator.is_local_main_process,miniters=20 )):
        ...
        batched_input = [] 
        if args.mode == "crop":
            imgs_,boxes = crop_image_grid(np.squeeze(imgs),np.squeeze(np.array(boxes.cpu()),axis=0),args.input_size)
        elif args.mode == "crop_with_overlap":
            imgs_,boxes = crop_image_grid_with_overlap(np.squeeze(imgs),np.squeeze(np.array(boxes.cpu()),axis=0),args.input_size)
        elif args.mode =="resize":
            imgs_,boxes = resize_img(np.squeeze(imgs),np.squeeze(np.array(boxes.cpu()),axis=0),args.input_size)
        else:
            imgs_,boxes = crop_image_grid(np.squeeze(imgs),np.squeeze(np.array(boxes.cpu()),axis=0),args.input_size)
        ...
        if args.mode == "crop":
            maskshqfinal = reconstruct_from_grid(maskshq,original_boxes,ori_onehotmask.shape,args.input_size)
        elif args.mode == "resize":
            maskshqfinal = reconstruct_from_resize(maskshq,original_boxes,ori_onehotmask.shape,args.input_size)
        elif args.mode == "crop_with_overlap":
            maskshqfinal = reconstruct_from_grid_with_overlap(maskshq,original_boxes,ori_onehotmask.shape,args.input_size)
        else:
            maskshqfinal = reconstruct_from_grid(maskshq,original_boxes,ori_onehotmask.shape,args.input_size)
    ...
    ts_metrics_list=torch.FloatTensor([hqall_iou,hqall_biou])
    final_metric = torch.mean(ts_metrics_list, dim=1)
    print('============================') 
    output_str = f"ros-sam-hqdecoder-iou: {final_metric[0]}, ros-sam-hqdecoder-biou: {final_metric[1]}\n"
...
def get_args_parser():
    ...
    parser.add_argument('--mode', default="crop_with_overlap", type=str,help='the mode of resizing input images.')
    parser.add_argument("--restore-model", type=str,default='result/temptest/checkpoint_9',
