...
import random
class SUNDRONE_Dataset(Dataset.Dataset):
    def __init__(self, datasetroot,split,use_argumentation):
        self.imgpath = os.path.join(datasetroot,split,'images')
        self.onehotpath = os.path.join(datasetroot,split,'masks')
    ...
    def __getitem__(self, index):
        ...
        onehotmask = skio.imread(os.path.join(self.onehotpath,self.img[index].replace('.jpg','.png')))
        ...
        sample ={'img':img,'onehotmask':onehotmask, 'filename':imagename}
        return  sample